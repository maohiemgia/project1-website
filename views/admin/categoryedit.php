...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
     ...
     foreach ($categoryArr as $cate) {
          if ($cate['category_name'] == $inp_catename) {
     ...
     if (empty($err) && $cateStatus == true) {
          ...
          `category`
      SET
          `category_name` = '$inp_catename'
      WHERE
          `category_id` = '$cateid'";
          queryDB($sql);

          header('location: ./category.php?mess=Sửa thành công');
     }
}
...
<body>
     ...
     <div class="row mx-0 manage-link-section p-3 ps-sm-5" style="min-height: 300px;">
          <h2 class="text-capitalize fw-bold fs-2 px-0">Sửa loại hàng</h2>
          ...
          <form action="" method="post">
               ...
               <input type="text" class="w-100 inputname" placeholder="nhập tên loại" name="catename" value="<?= isset($categoryById['category_name']) ? $categoryById['category_name'] : '' ?>">
